...
def send_zalo_request_phone(tenant_zalo_id: str) -> bool:
 ...
 def send_zalo_request_phone_v2(user_id: str):
    ...
    # Payload chuẩn tin nhắn kèm Nút bấm tương tác
    payload = {
        "recipient": {
            "user_id": user_id
        },
        "message": {
            "text": "Chào bạn! Để hoàn tất đăng ký / đặt lịch, vui lòng bấm nút bên dưới để chia sẻ Số điện thoại xác thực nhé:",
            "attachment": {
                "type": "template",
                "payload": {
                    "template_type": "request_user_info",
                    "elements": [
                        {
                            "title": "Xác thực thông tin tài khoản",
                            "subtitle": "Chúng tôi cần SĐT của bạn để liên hệ xác nhận phòng.",
                            "image_url": "https://stc-oa.zdn.vn/templates/v1/images/img_sample.png"
                        }
                    ]
                }
            }
    ...
    try:
        ...
        print(f"❌ [API ERROR] Lỗi gửi tin nhắn tương tác Zalo: {e}")
        return None